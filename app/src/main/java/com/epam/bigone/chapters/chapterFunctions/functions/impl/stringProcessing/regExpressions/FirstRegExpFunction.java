 ...
 **/
public class FirstRegExpFunction implements Function<ParagraphMap> {
	...
	public void calcFunction(ParagraphMap paragraph) {
		...
		stringList.setFirstList(Arrays.stream(paragraph.getPattern().split(paragraph.getStringFromFile()))
		...
		paragraph.setParagraphMap(paragraph.sortByEntrySet(
				paragraph.getEntrySet(), SortEnum.SORT_BY_ENTRY_VALUE));
	}
	...
	public String getResultString(ParagraphMap paragraph) {
		...
		paragraph.getParagraphMap().forEach((k, v) ->
			...
			key.forEach(o -> resultList.add(format(" - [%2s]", o)));
		}));
