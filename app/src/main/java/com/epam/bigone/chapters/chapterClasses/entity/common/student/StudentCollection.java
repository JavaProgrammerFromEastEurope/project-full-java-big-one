package com.epam.bigone.chapters.chapterClasses.entity.common.student;
...
public class StudentCollection {
	...
	public static void averageMarkGroup(List<Student> studentList, int score) {
		...
		Map<Boolean, List<Student>> studentListByValue = studentList.stream()
				.collect(
						Collectors.partitioningBy(s -> s.getPersonAverageScore() >= score));

		studentListByValue.forEach((k, v) -> out.printf(
				"Key:%s  %s%n", k, v.stream()
